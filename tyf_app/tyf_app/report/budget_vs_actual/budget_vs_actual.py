# Copyright (c) 2013, Tamdeen Youth Foundation and contributors
...
def get_final_data(dimension, dimension_items, filters, period_month_ranges, data, DCC_allocation):
	...
	for budget_for, monthwise_data in iteritems(dimension_items):
		if filters['budget_against'] == "Cost Center":
			row = [dimension, budget_for]
			data = get_final_bl_data(row, monthwise_data, filters, period_month_ranges, data, DCC_allocation)

		else:
			for account, monthly_data in iteritems(monthwise_data):
				row = [dimension, budget_for, account]
				data = get_final_bl_data(row, monthly_data, filters, period_month_ranges, data, DCC_allocation)

	return data


def get_final_bl_data(row, monthwise_data, filters, period_month_ranges, data, DCC_allocation):
	totals = [0, 0, 0]
...
def get_dimension_target_details(filters):
	...
	if filters.budget_against == "Project":
		return frappe.db.sql(
			"""
				select
					b.project as budget_against,
					CONCAT(bl.code," - ",bl.title) as budget_line,
					bl.account,
					b.monthly_distribution,
					bl.total_cost as budget_amount,
					b.fiscal_year
				from
					`tabBudget` b,
		...
		return frappe.db.sql(
			"""
				select
					b.cost_center as budget_against,
					b.monthly_distribution,
					ba.account,
					ba.budget_amount,
					b.fiscal_year
...
def get_actual_details(name, filters):
	...
	if filters.get("budget_against") == "Cost Center":
		...
		ac_details = frappe.db.sql(
			...
			""".format(tab=filters.budget_against, budget_against=budget_against, lft=cc_lft, rgt=cc_rgt),
	...
	for ccd in dimension_target_details:
		...
		for month_id in range(1, 13):
			...
			if filters.budget_against == "Cost Center":
				...
				budget_for = ccd.account
				tav_dict = cam_map[ccd.budget_against][ccd.account][ccd.fiscal_year][month]
			else:
				cam_map.setdefault(ccd.budget_against, {}
				).setdefault(
					ccd.budget_line, {}
				).setdefault(
					ccd.account, {}
				...
				budget_for = ccd.budget_line
				tav_dict = cam_map[ccd.budget_against][ccd.budget_line][ccd.account][ccd.fiscal_year][month]
			month_percentage = (
